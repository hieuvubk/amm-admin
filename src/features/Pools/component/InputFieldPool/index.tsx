...
import { maxDigitsAfterDecimalRegex, floatNumberRegex, numberRegex } from 'src/pages/CreatePoolRequest/constants';
      ...
      <div className={cx('password-field-container')}>
        <TextField
          ...
          onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
            if (limitDigitAfterPeriod) {
              if (maxDigitsAfterDecimalRegex(limitDigitAfterPeriod).test(event.target.value)) {
            ...
            } else if (floatNumberRegex.test(event.target.value)) onChange && onChange(event);
          }}
